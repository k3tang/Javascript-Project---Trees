   ...
   (async ($d3) => {
    ...
    let svg = d3.select("div#contents")
       ...
       for (let i = radius.length; i > 0; i--) {
        ...
        for (let i = radius.length + 1; i > 1; i--) {
         let ele = document.getElementById(i)
         console.log(ele, "ele")
        //     ele.onscroll =  function () { 
        //     return tip.style("visibility", "visible")
        // .html(`<div class="tip-text"><div class="tree-age">Methuselah is <div class="age-text">${events[i - 1].age} years old</div></div><p class="tip-header">In ${events[i - 1].year}</p><p class="tip-body">${events[i - 1].event}</p><a class="tip-link" href=${events[i - 1].website} target="_blank" rel="noopener noreferrer">Learn More</a></div>
        // `)}
        }
